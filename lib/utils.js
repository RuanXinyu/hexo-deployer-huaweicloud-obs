		...
		if(param[key] !== null && param[key] !== undefined){
			...
			if(meta.location === 'uri'){
				...
				if(meta.type === 'object'){
					...
					let val = this[key + 'Adapter'](param[key]);
					if(val){
...
Utils.prototype.doAuth = function(opt, methodName) {
	...
	for(let key in opt.headers){
		if (key.toLowerCase().indexOf(this.headerPrefix) === 0){
	...
	for(let i=0;i<temp.length;i++){
		let key = temp[i];
		let val = key.indexOf(this.headerMetaPrefix) === 0 ? opt.headers[key].trim() : opt.headers[key];
...
Utils.prototype.parseCommonHeaders = function(opt, headers){
	for(let key in commonHeaders){
		opt.InterfaceResult[commonHeaders[key]] = headers[key];
	}
	opt.InterfaceResult.RequestId = headers[this.headerPrefix + 'request-id'];
	opt.InterfaceResult.Id2 = headers[this.headerPrefix + 'id-2'];
};

Utils.prototype.contrustCommonMsg = function(opt, obj, headers, methodName){
	opt.InterfaceResult = {};
	var log = this.log;
	this.parseCommonHeaders(opt, headers);
...
Utils.prototype.getRequest = function(methodName, serverback, dstFile, bc, saveAsStream, readable){
	var opt = {};
	var log = this.log;
	var model = this.signature === 'obs' ? obsModel[methodName + 'Output'] : v2Model[methodName + 'Output'];
	model = model || {};
...
Utils.prototype.createV2SignedUrlSync = function(param){
	...
	for(let i=0;i<queryParamsKeys.length;i++){
		...
		if(allowedResourceParameterNames.indexOf(key.toLowerCase())>=0 || key.toLowerCase().indexOf(this.headerPrefix) === 0){
			flag = true;
...
Utils.prototype.createPostSignatureSync = function(param){
	...
	if(this.securityToken && !formParams[this.headerPrefix + 'security-token']){
		formParams[this.headerPrefix + 'security-token'] = this.securityToken;
	}
	...
	for(let key in formParams){
		if(key){
			...
			if(allowedResponseHttpHeaderMetadataNames.indexOf(key) < 0 && conditionAllowKeys.indexOf(key) < 0 && key.indexOf(this.headerPrefix) !== 0){
				continue;
			}
